...
awk -F'\t' 'BEGIN{OFS="\t"} {print $1, $2 * 0.65}' "${dump_dir}/threshold_h40.tsv" > "${dump_dir}/threshold_h40_0.65wg.tsv"

thresholds_file="${dump_dir}/threshold_h40_0.65wg.tsv"
#use 65% threshold for historical

summary_out=${out}/summaryhistorical40_tailocin_after0.65wgavgdepth_filter.tsv
...
#historicalLoop through each isolate .jf
for jf in "$wd"/isolate_jf/*.jf; do
    iso_name=$(basename "$jf" .jf)
    # Skip if name starts with lowercase p
    if [[ "$iso_name" == p* ]]; then
      continue
    fi
    ...
    if [[ -z "$cutoff" ]]; then
        echo "Warning: No cutoff found for $iso_name — skipping"
        continue
    fi
